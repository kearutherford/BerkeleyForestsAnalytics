...
FWD %>% filter(time == "post", site == "400", plot == "2")
```
...
fwd_sum <- FWD %>%
  ...
  mutate(w_1h = sc_length_1h/(sum(sc_length_1h)/n()),
         ...
         w_load_fwd = w_load_1h + w_load_10h + w_load_100h) %>% # Note: if you have a missing 1h, 10h, or 100h load estimate, the total fine woody debris load estimate should be NA
...
* \(y_i\) is the value per unit area based on line transect i (e.g., Mg/ha)
