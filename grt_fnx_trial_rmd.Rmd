...
if(sawlog == "fia") {
  ...
  data$sawlog_d <- ifelse(data$type == "S" & data$dbh >= 9, 7, ifelse(data$type == "H" & data$dbh >= 11, 9, NA))
  
} else if (sawlog == "CA") {
  
  data$sawlog_d <- ifelse(data$type == "S" & data$dbh >= 9, 6, ifelse(data$type == "H" & data$dbh >= 11, 8, NA))
...
NSVB <- function(data) {
  ...
  data$sawlog_wood_bio <- as.double(NA)
  data$sawlog_bark_bio <- as.double(NA)
  data$sawlog_total_bio <- as.double(NA)
  ...
  data$sawlog_wood_c <- as.double(NA)
  data$sawlog_bark_c <- as.double(NA)
  data$sawlog_total_c <- as.double(NA)
  ...
  for (i in 1:n) {
    ...
    # If sawlog 
    if((data$type[i] == "S" && data$dbh[i] >= 9) || (data$type[i] == "H" && data$dbh[i] >= 11)) {
      
      EQ_7_hs <- uniroot(EQ_7, c(0,data$ht1[i]), tol = 0.001, maxiter = 100000, d = data$sawlog_d[i], table_data_1 = Table_S3_Pull, table_data_2 = Table_S4_Pull, tree_data = data[i,])
      h_s <- EQ_7_hs$root 
      
      R_s <- EQ_6(h_s, Table_S5_Pull, data[i,])
    ...
    if(data$path[i] == "1") {
      ...
      if((data$type[i] == "S" && data$dbh[i] >= 9) || (data$type[i] == "H" && data$dbh[i] >= 11)) {
        ...
        data$sawlog_wood_bio[i] <- Wsaw_ib
        data$sawlog_bark_bio[i] <- Wstump_bk
        data$sawlog_total_bio[i] <- Wsaw_ob
    ...
    # follow path 4 (dead, with broken top) ----------------------------------->
      ...
      if((data$type[i] == "S" && data$dbh[i] >= 9) || (data$type[i] == "H" && data$dbh[i] >= 11)) {
        ...
        if(data$ht2[i] < h_s) {
          ...
          R_s_2 <- EQ_6(data$ht2[i], Table_S5_Pull, data[i,])
          Vsaw_ib_Sound <- (R_s_2*Vtot_ib_Gross) - (R_1*Vtot_ib_Gross)
          Vsaw_bk_Sound <- (R_s_2*Vtot_bk_Gross) - (R_1*Vtot_bk_Gross)
          Vsaw_ob_Sound <- Vsaw_ib_Sound - Vsaw_bk_Sound
      ...
      if((data$type[i] == "S" && data$dbh[i] >= 9) || (data$type[i] == "H" && data$dbh[i] >= 11)) {
        ...
        Wsaw_ib <- Vsaw_ib_Sound*WDSG_Adj*62.4
        Wsaw_bk <- Vsaw_bk_Sound*BKSG_Adj*62.4
        Wsaw_ob <- Wsaw_ib + Wsaw_bk 
    ...
    data$sawlog_wood_c[i] <- data$sawlog_wood_bio[i]*CF
    data$sawlog_bark_c[i] <- data$sawlog_bark_bio[i]*CF
    data$sawlog_total_c[i] <- data$sawlog_total_bio[i]*CF
    data$sawlog_top_c[i] <- data$sawlog_top_bio[i]*CF
